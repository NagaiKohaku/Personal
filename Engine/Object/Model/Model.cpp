...
void Model::Initialize(ModelCommon* modelCommon, MeshType type, const std::string& filePath) {
	...
	for (auto& part : modelParts_) {
		...
		//書き込むためのアドレスを取得する
		part.materialResource->Map(0, nullptr, reinterpret_cast<void**>(&part.material));

		//マテリアルデータの設定
		part.material->color = Vector4(1.0f, 1.0f, 1.0f, 1.0f);
		part.material->enableLighting = true;
		part.material->uvTransform = MakeIdentity4x4();
		part.material->shininess = 50.0f;
		part.material->environmentCoefficient = 1.0f;

		if (part.textureFilePath.empty()) {
			part.textureFilePath = kDefaultTexterPath_;
			part.textureIndex = TextureManager::GetInstance()->GetSrvIndex(kDefaultTexterPath_);
		}
	}
}
	...
	while (std::getline(file, line)) {
		...
		if (identifier == "mtllib") {
			...
			if (materialMap.contains(materialName)) {
				...
				modelParts_[currentPartIndex].textureIndex = TextureManager::GetInstance()->GetSrvIndex(texturePath);
			}
		}
	}
}
...
void MyEngine::Model::SetTextureFilePath(const std::string filePath) {
