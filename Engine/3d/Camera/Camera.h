...
/// </summary>
/// <remarks>
/// - ビュー行列、プロジェクション行列、ビュープロジェクション行列を管理し、描画に必要なカメラ空間変換を提供します。  
/// - 通常カメラとデバッグカメラの2種類のモードをサポートします。  
/// - デバッグカメラモードではマウス操作による自由視点移動やズーム操作が可能です。  
/// - 通常カメラモードでは、設定された 3D オブジェクトを追従するカメラ挙動を実現します。  
/// </remarks>
class Camera {
